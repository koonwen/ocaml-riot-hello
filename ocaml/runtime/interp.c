...
next_instr:
  ...
  Instruct(ACC0) : accu = sp[0];
  Next;
  Instruct(ACC1) : accu = sp[1];
  Next;
  Instruct(ACC2) : accu = sp[2];
  Next;
  Instruct(ACC3) : accu = sp[3];
  Next;
  Instruct(ACC4) : accu = sp[4];
  Next;
  Instruct(ACC5) : accu = sp[5];
  Next;
  Instruct(ACC6) : accu = sp[6];
  Next;
  Instruct(ACC7) : accu = sp[7];
  Next;
  ...
  Instruct(PUSHACC) :
                          *--sp = accu;
  /* Fallthrough */
  Instruct(ACC) : accu = sp[*pc++];
  Next;

  Instruct(POP) : sp += *pc++;
  Next;
  Instruct(ASSIGN) : sp[*pc++] = accu;
  accu = Val_unit;
  Next;

  /* Access in heap-allocated environment */

  Instruct(ENVACC1) : accu = Field(env, 1);
  Next;
  Instruct(ENVACC2) : accu = Field(env, 2);
  Next;
  Instruct(ENVACC3) : accu = Field(env, 3);
  Next;
  Instruct(ENVACC4) : accu = Field(env, 4);
  Next;
  ...
  Instruct(PUSHENVACC) :
                             *--sp = accu;
  /* Fallthrough */
  Instruct(ENVACC) : accu = Field(env, *pc++);
  Next;
  ...
  Instruct(PUSHOFFSETCLOSURE) :
                                    *--sp = accu; /* fallthrough */
  Instruct(OFFSETCLOSURE) : accu = env + *pc++ * sizeof(value);
  Next;

  Instruct(PUSHOFFSETCLOSUREM3) :
                                      *--sp = accu; /* fallthrough */
  Instruct(OFFSETCLOSUREM3) : accu = env - 3 * sizeof(value);
  Next;
  Instruct(PUSHOFFSETCLOSURE0) :
                                     *--sp = accu; /* fallthrough */
  Instruct(OFFSETCLOSURE0) : accu = env;
  Next;
  Instruct(PUSHOFFSETCLOSURE3) :
                                     *--sp = accu; /* fallthrough */
  Instruct(OFFSETCLOSURE3) : accu = env + 3 * sizeof(value);
  Next;
  ...
  Instruct(PUSHGETGLOBAL) :
                                *--sp = accu;
  /* Fallthrough */
  Instruct(GETGLOBAL) : accu = Field(caml_global_data, *pc);
  pc++;
  Next;
  ...
  Instruct(SETGLOBAL) : caml_modify(&Field(caml_global_data, *pc), accu);
  accu = Val_unit;
  pc++;
  Next;
  ...
  Instruct(PUSHATOM0) :
                            *--sp = accu;
  /* Fallthrough */
  Instruct(ATOM0) : accu = Atom(0);
  Next;

  Instruct(PUSHATOM) :
                           *--sp = accu;
  /* Fallthrough */
  Instruct(ATOM) : accu = Atom(*pc++);
  Next;
  ...
  Instruct(GETFIELD0) : accu = Field(accu, 0);
  Next;
  Instruct(GETFIELD1) : accu = Field(accu, 1);
  Next;
  Instruct(GETFIELD2) : accu = Field(accu, 2);
  Next;
  Instruct(GETFIELD3) : accu = Field(accu, 3);
  Next;
  Instruct(GETFIELD) : accu = Field(accu, *pc);
  pc++;
  Next;
  ...
  Instruct(SETFIELD0) : caml_modify(&Field(accu, 0), *sp++);
  accu = Val_unit;
  Next;
  Instruct(SETFIELD1) : caml_modify(&Field(accu, 1), *sp++);
  accu = Val_unit;
  Next;
  Instruct(SETFIELD2) : caml_modify(&Field(accu, 2), *sp++);
  accu = Val_unit;
  Next;
  Instruct(SETFIELD3) : caml_modify(&Field(accu, 3), *sp++);
  accu = Val_unit;
  Next;
  Instruct(SETFIELD) : caml_modify(&Field(accu, *pc), *sp++);
  accu = Val_unit;
  pc++;
  Next;
  Instruct(SETFLOATFIELD) : Store_double_flat_field(accu, *pc, Double_val(*sp));
  accu = Val_unit;
  sp++;
  pc++;
  Next;
  ...
  Instruct(VECTLENGTH) :
  {
    ...
    if (Tag_val(accu) == Double_array_tag)
      size = size / Double_wosize;
    accu = Val_long(size);
    Next;
  }
  Instruct(GETVECTITEM) : accu = Field(accu, Long_val(sp[0]));
  sp += 1;
  Next;
  Instruct(SETVECTITEM) : caml_modify(&Field(accu, Long_val(sp[0])), sp[1]);
  accu = Val_unit;
  sp += 2;
  Next;

  /* Bytes/String operations */
  Instruct(GETSTRINGCHAR) : Instruct(GETBYTESCHAR) : accu = Val_int(Byte_u(accu, Long_val(sp[0])));
  sp += 1;
  Next;
  Instruct(SETBYTESCHAR) : Byte_u(accu, Long_val(sp[0])) = Int_val(sp[1]);
  sp += 2;
  accu = Val_unit;
  Next;
  ...
  Instruct(BRANCH) : pc += *pc;
  Next;
  Instruct(BRANCHIF) : if (accu != Val_false) pc += *pc;
  else pc++;
  Next;
  Instruct(BRANCHIFNOT) : if (accu == Val_false) pc += *pc;
  else pc++;
  Next;
  Instruct(SWITCH) :
  {
  ...
  Instruct(BOOLNOT) : accu = Val_not(accu);
  Next;

  /* Exceptions */

  Instruct(PUSHTRAP) : sp -= 4;
  Trap_pc(sp) = pc + *pc;
  ...
  Instruct(POPTRAP) : if (caml_something_to_do)
  {
  ...
  Instruct(RAISE_NOTRACE) : Check_trap_barrier;
  goto raise_notrace;

  Instruct(RERAISE) : Check_trap_barrier;
  if (Caml_state->backtrace_active)
  {
    *--sp = (value)(pc - 1);
  ...
  Instruct(RAISE) : Check_trap_barrier;
  if (Caml_state->backtrace_active)
  {
    *--sp = (value)(pc - 1);
    caml_stash_backtrace(accu, sp, 0);
...
raise_notrace:
  if ((char *)Caml_state->trapsp >= (char *)Caml_state->stack_high - initial_sp_offset)
  {
    ...
    Caml_state->extern_sp = (value *)((char *)Caml_state->stack_high - initial_sp_offset);
    caml_callback_depth--;
...
process_actions:
  ...
  Instruct(C_CALL1) : Setup_for_c_call;
  accu = Primitive(*pc)(accu);
  Restore_after_c_call;
  pc++;
  Next;
  Instruct(C_CALL2) : Setup_for_c_call;
  accu = Primitive(*pc)(accu, sp[2]);
  Restore_after_c_call;
  sp += 1;
  pc++;
  Next;
  Instruct(C_CALL3) : Setup_for_c_call;
  accu = Primitive(*pc)(accu, sp[2], sp[3]);
  Restore_after_c_call;
  sp += 2;
  pc++;
  Next;
  Instruct(C_CALL4) : Setup_for_c_call;
  accu = Primitive(*pc)(accu, sp[2], sp[3], sp[4]);
  Restore_after_c_call;
  sp += 3;
  pc++;
  Next;
  Instruct(C_CALL5) : Setup_for_c_call;
  ...
  Instruct(CONST0) : accu = Val_int(0);
  Next;
  Instruct(CONST1) : accu = Val_int(1);
  Next;
  Instruct(CONST2) : accu = Val_int(2);
  Next;
  Instruct(CONST3) : accu = Val_int(3);
  Next;
  ...
  Instruct(PUSHCONSTINT) :
                               *--sp = accu;
  /* Fallthrough */
  Instruct(CONSTINT) : accu = Val_int(*pc);
  pc++;
  Next;

  /* Integer arithmetic */

  Instruct(NEGINT) : accu = (value)(2 - (intnat)accu);
  Next;
  Instruct(ADDINT) : accu = (value)((intnat)accu + (intnat)*sp++ - 1);
  Next;
  Instruct(SUBINT) : accu = (value)((intnat)accu - (intnat)*sp++ + 1);
  Next;
  Instruct(MULINT) : accu = Val_long(Long_val(accu) * Long_val(*sp++));
  Next;
  ...
  Instruct(ANDINT) : accu = (value)((intnat)accu & (intnat)*sp++);
  Next;
  Instruct(ORINT) : accu = (value)((intnat)accu | (intnat)*sp++);
  Next;
  Instruct(XORINT) : accu = (value)(((intnat)accu ^ (intnat)*sp++) | 1);
  Next;
  Instruct(LSLINT) : accu = (value)((((intnat)accu - 1) << Long_val(*sp++)) + 1);
  Next;
  Instruct(LSRINT) : accu = (value)((((uintnat)accu) >> Long_val(*sp++)) | 1);
  Next;
  Instruct(ASRINT) : accu = (value)((((intnat)accu) >> Long_val(*sp++)) | 1);
  Next;

#define Integer_comparison(typ, opname, tst)                    \
  Instruct(opname) : accu = Val_int((typ)accu tst(typ) * sp++); \
  Next;
                                                      ...
                                                      Integer_branch_comparison(intnat, BGEINT, >=, ">=")
                                                          Integer_branch_comparison(uintnat, BULTINT, <, "<")
                                                              Integer_branch_comparison(uintnat, BUGEINT, >=, ">=")
                                                                  ...
                                                                  Instruct(OFFSETINT) : accu += *pc << 1;
  pc++;
  Next;
  Instruct(OFFSETREF) : Field(accu, 0) += *pc << 1;
  accu = Val_unit;
  pc++;
  Next;
  Instruct(ISINT) : accu = Val_long(accu & 1);
  Next;
...
#define Lookup(obj, lab) Field(Field(obj, 0), Int_val(lab))
  ...
  Instruct(GETMETHOD) : accu = Lookup(sp[0], accu);
  Next;
...
//#define CAML_METHOD_CACHE
  ...
  Instruct(STOP) : Caml_state->external_raise = initial_external_raise;
  Caml_state->extern_sp = sp;
  caml_callback_depth--;
  return accu;

  Instruct(EVENT) : if (--caml_event_count == 0)
  {
    Setup_for_debugger;
  ...
  Instruct(BREAK) : Setup_for_debugger;
...
default:
  ...
  __assume(0);
#else
  caml_fatal_error("bad opcode (%" ARCH_INTNAT_PRINTF_FORMAT "x)",
                   (intnat) * (pc - 1));
#endif
}
}
